...
export class TelegramAdapter implements NotificationAdapter {
    ...
    setupListeners(): void {
        this.bot.start((ctx) => ctx.replyWithMarkdown('Hello fellow Lisker 👋!\n' +
        ...
        this.bot.command('delegate', (ctx) => {
            if (ctx.message.text.split(' ').length != 2) {
                ...
                const dbEntry = this.db.get('delegates').find({id: ctx.chat.id, name: delegate}).value();
                if (dbEntry == null) {
    ...
    handleDelegateDroppedTop(delegate: DelegateDetails): void {
        const recipients = this.db.get('delegates').filter({name: delegate.username}).value();
        if (!recipients) return;

        recipients.forEach((recipient) => {
            this.bot.telegram.sendMessage(recipient.id, '👋 *Bye Bye* 👋 \n' +
    ...
    handleDelegateNewTop(delegate: DelegateDetails): void {
        ...
        recipients.forEach((recipient) => {
            this.bot.telegram.sendMessage(recipient.id, '🎊 *Congratulations* 🎊 \n' +
    ...
    handleDelegateRankChanged(delegate: DelegateDetails, delta: number): void {
        ...
        recipients.forEach((recipient) => {
            this.bot.telegram.sendMessage(recipient.id, '*Rank change* \n' +
                'Delegate `' + delegate.username + '` \n' +
                'Rank: `' + delegate.rank + ' (' + (delta > 0 ? '+' : '-') + Math.abs(delta) + ')`\n' +
                'Network: `' + this.network + '`', {parse_mode: 'markdown'});
        })
    }
